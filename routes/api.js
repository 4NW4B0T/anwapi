...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di wa.me/6282281917822 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
    ...
    invalidKey: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'Api Key Nya Anjg, Kalau Ga Ada Chat Aja wa.me/+6282281917822, Apikey Gratis'
    },
    invalidlink: {
        status: false,
        creator: `${creator}`,
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------AnwaGanteng'+'ANWA--GANS';
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'anwapikey270624') return res.json(loghandler.invalidKey)
...
router.get('/textmaker/senja', async (req, res, next) => {
	...
	if(apikeyInput != 'anwapikey270624') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'coffee-cup' && theme != 'coffee-cup2' && theme != 'wolf-metal') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
